...
public class TStringGenerator
{
    ...
    public static void main(String[] args)
    {
        ...
        {
            System.out.println("No files found that have to be parsed.\nNothing to be done...");
        }
        else if (TStringConfig.getMode() == Mode.CREATE)
        {
            new TStringsCreator(TStringConfig.getRootDir(), TStringConfig.getBackupDir()).makeTString(parseArray);
        }
        else if (TStringConfig.getMode() == Mode.UPDATE)
        {
            new TStringsUpdater(TStringConfig.getRootDir(), TStringConfig.getBackupDir()).makeTString(parseArray);
        }
    }
}
